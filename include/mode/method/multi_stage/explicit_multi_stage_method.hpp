...
namespace mode
{
template <typename tableau_type_, problem problem_type_>
class explicit_multi_stage_method
{
public:
  ...
  __device__ __host__ static constexpr auto apply(const problem_type& problem, const typename problem_type::time_type step_size)
  {
    ...
    constexpr_for<0, butcher_tableau::stages_v<tableau_type>, 1>([&problem, &step_size, &stages] (auto i)
    {
      ...
      {
        sum += std::get<j>(stages) * std::get<triangular_number<i.value - 1> + j.value>(butcher_tableau::a_v<tableau_type>);
      });
      std::get<i.value>(stages) = problem.function(problem.time + std::get<i.value>(butcher_tableau::c_v<tableau_type>) * step_size, problem.value + sum * step_size);
    });
