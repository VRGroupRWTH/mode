...
struct quantity_operations 
{
  ...
  __device__ __host__ static constexpr void        for_each(const function_type& function, const value_types&... values)
  {
  ...
  __device__ __host__ static constexpr std::size_t size    (const value_type& value)
  {
    if constexpr (is_scalar_v<value_type>)
      return 1;
    else
